...
LayerSnapshot make_fake_snapshot(uint32_t seq_id, int layer_idx) {
    ...
    switch (layer_idx % 4) {
        case 0: snap.type=LayerType::Embedding;  snap.device=ComputeDevice::CUDA; break;
        case 1: snap.type=LayerType::Attention;  snap.device=ComputeDevice::CUDA; break;
        case 2: snap.type=LayerType::MLP;        snap.device=ComputeDevice::CUDA; break;
        case 3: snap.type=LayerType::LayerNorm;  snap.device=ComputeDevice::CPU;  break;
    }
    snap.attn_rows = 8; snap.attn_cols = 8;
    for (int r=0; r<8; r++)
        for (int c=0; c<8; c++)
            snap.attn_matrix[r][c] = std::exp(-std::abs(r-c)*0.5f);
